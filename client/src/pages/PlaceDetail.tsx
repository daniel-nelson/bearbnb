...
export default function PlaceDetail() {
  ...
  const [reviewableBookingId, setReviewableBookingId] = useState("");
  ...
  useEffect(() => {
    ...
    // A returning signed-in guest can review any of their bookings for this
    // place that doesn't already have a review, even across page remounts.
    async function loadReviewableBooking(placeId: string) {
      if (!authUid) {
        setReviewableBookingId("");
        return;
      }
      ...
      if (bookings.error || !bookings.data || reviews.error || !reviews.data) {
        setReviewableBookingId("");
        return;
      }

      const reviewedBookingIds = new Set(
        reviews.data.results.map((review) => review.bookingId),
      );
      const reviewable = bookings.data.results.find(
        (booking) =>
          booking.placeId === placeId &&
          !reviewedBookingIds.has(booking.id),
      );

      setReviewableBookingId(reviewable?.id ?? "");
    }

    void loadPlace(id);
    void loadAvailability(id);
    void loadReviews(id);
    void loadReviewableBooking(id);
          ...
          <aside className="mt-8 self-start border border-[#deded8] bg-white lg:sticky lg:top-6 lg:mt-0">
            ...
            {user ? (
              <BookingForm
                ...
                onBookingConfirmed={setReviewableBookingId}
              />
            ) : (
              <div className="p-4">
                <p className="text-sm font-medium uppercase tracking-[0.12em] text-[#707069]">
