...
export default class FirebaseAuth {
  ...
  public static async userFromBearerToken(authorizationHeader: string | undefined): Promise<User | null> {
    ...
    const existingUser = await User.removeAllDefaultScopes().findBy({ firebaseUid: payload.uid })
    if (existingUser?.deletedAt) return null
    if (existingUser) return existingUser

    const unlinkedUserByEmail = await User.removeAllDefaultScopes().findBy({ email: payload.email })
  ...
  private static verifiedPayload(payload: FirebaseUserPayload) {
  ...
  private static async verifyToken(token: string): Promise<FirebaseUserPayload | null> {
    ...
    const testPayload = this.testPayload(token)
  ...
  private static testPayload(token: string): FirebaseUserPayload | null {
    ...
    try {
      ...
      const payload = JSON.parse(Buffer.from(encodedPayload, 'base64url').toString()) as FirebaseUserPayload
